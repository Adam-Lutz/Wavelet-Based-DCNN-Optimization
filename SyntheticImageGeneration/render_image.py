...
class ImageGenerator(ShowBase):
    def __init__(self, num_imgs, models_per_img, img_wid, img_hgt, model_path, out_dir_path, stereo, eye_sep, depth, verbose):
        ...
        self.x_lower_lim =  -2  #Limits for object position on the x axis
        self.x_upper_lim = 2
        self.y_lower_lim = 35  #Limits for object position on the y axis (smaller value = closer to camera)
        self.y_upper_lim = 40
        self.scale_lower_lim = -2 #Limits for object scale
        self.scale_upper_lim = 5
        ...
        self.placeBackground("models/cornfield")  #Default background model is 'environment'              
        ...
        for i in range(self.num_imgs):    #Loop for generating each image
            ...
            if(self.stereo):
                ...
                base.camera.setPos(-1*(self.eye_separation/2),-40,20) #Set camera position to the left
                base.camera.setHpr(0, -25, 0) #Adjust position of background model in scene
                self.renderToPNM(filename=file_name1, verbose=self.verbose) #Produce a rendered image        

                ''' Render right-eye image '''
                base.camera.setPos(self.eye_separation/2,-40,20)      #Set camera position to the right
                base.camera.setHpr(0, -25, 0) #Adjust position of background model in scene
                ...
                base.camera.setPos(0, 30, 4) #A non-stereo image is being generated, so center the camera
                base.camera.setHpr(0, -15, 0) #Adjust position of background model in scene
                self.renderToPNM(filename=file_name1, verbose=self.verbose) #Produce a rendered image        
    ...
    def placeBackground(self, model_name):
        ...
        self.scene.setScale(.25, .25, .25) #Adjust scaling of background model
        self.scene.setPos(0, 42, 2) #Adjust position of background model in scene
    ...
    def placeModel(self, model_name, model_num=None, verbose=None):        
        ...
        bg_height = bg_height  / 2                 #Say height of model in background should be about 1/24th of the total background height
        ...
        model.setPos(pos_origin + pos_x, pos_origin + pos_y, 2.5)    #Set the position of the model in the scene
        rotation = random.uniform(0,5000)
        model.setHpr(rotation,   0, 0)
...
if __name__ == "__main__":        
    ...
    parser.add_argument("--eye_sep", help="distance of separation between two stereo images", metavar="EYE_SEPARATION", type=float, default=.4)
    ...
    parser.add_argument("--model_path", help="path to the 3d model to be rendered", default="models/Car")    
